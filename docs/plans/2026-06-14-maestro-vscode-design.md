...
- **Milestone 4 (VS Code cockpit) — DONE (2026-06-14, branch `m4-vscode-cockpit`).** The thin vertical slice that makes Maestro a real, installable VS Code extension: spawn an agent, watch it stream in an isolated worktree, review its diff, Merge or Discard. Built via subagent-driven TDD (implementer + spec + code-quality review per substantive task, plus a final holistic integration review). Two new packages, keeping the "pure brain, thin shell" discipline. (A) `@maestro/cockpit` — the pure presenter: a reducer `(model, OrchestratorEvent) => model`, `selectState` (attention-first ordering so cards needing a decision float up), an output cap, and the typed host↔webview message protocol. 12 tests, zero VS Code. (B) `@maestro/extension` — the real extension: a `createCockpit` controller (unit-tested against a fake orchestrator; the real `Orchestrator` is structurally assignable to its `OrchestratorLike`), a native `RosterTreeDataProvider`, a `StageWebviewPanel`, a vanilla-TS webview client rendered from typed state snapshots, and an `activate()` that wires the real `Orchestrator` + `GitWorkspaceManager` + `CopilotAdapter` with a seeded Implementer role. esbuild bundles the host (CJS, `vscode` external) and the webview (browser IIFE); tsc is typecheck-only. Pure helpers split from VS Code-importing files (`roster-map.ts`, `html.ts`, `render.ts`) so they unit-test without a host. Security review hardened the webview: all engine output escaped, strict nonce-gated CSP, `card.id` escaped in attributes, Web Crypto CSPRNG nonce (browser-bundle-safe). Copilot's honest capability degradation shows an "auto" badge and no approve/steer controls (it is `approvals`/`steerable: false`). **Full M4 gate: 47 core + 12 cockpit + 20 adapter + 17 workspace + 16 extension = 112 tests green, tsc clean across all five packages, all five build (extension emits `dist/extension.js` + `dist/webview/{main.js,style.css}`).** Plan: `2026-06-14-maestro-m4-vscode-cockpit.md`. Deferred (tracked): per-tool approval UI + steering box + send-back-with-feedback (need a steerable/approving engine), `.conductor/` YAML roles/teams, persistence/rehydration across reload, conflict-resolve-in-editor, and a `@vscode/test-electron` integration harness (the glue is verified by build + typecheck + manual F5).
- **Next:** Milestone 5 — parallelism + the generic ACP adapter (lights up Gemini + Copilot-ACP from one implementation and validates model-agnosticism against a second protocol shape). The superseded Claude-adapter plan (`2026-06-14-maestro-claude-adapter.md`) is retained for its Agent-SDK research and auth finding.
