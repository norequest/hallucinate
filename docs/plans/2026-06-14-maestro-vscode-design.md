...
- **Milestone 3 (Workspace Manager) — PLANNED (`2026-06-14-maestro-workspace-manager.md`, designed via 3 parallel specialist agents).** Two phases: (A) `@maestro/core` gains a feature-detected `WorkspaceManager` capability (`diff`/`merge`/`discard`), diff-on-done, `merge`/`discard` + a non-terminal `conflict` state — all additive, keeps the 55 existing tests green; (B) a new `@maestro/workspace` package with `GitWorkspaceManager` driving real `git worktree`/diff/merge via an injected `GitRunner`. Key call: `cleanup()` wires to *resolution* (merge/discard), never terminal, so the worktree survives as the review surface. Deferred: rebase/stale-base, send-conflict-back, merge mutex, PR-mode. Turns the streamed Copilot runs into the "review the diff, merge" product.
- **Next:** execute M3, then Milestone 4 — the VS Code cockpit (roster, live panes, diff review wired to `orch.merge`/`orch.discard`). The superseded Claude-adapter plan (`2026-06-14-maestro-claude-adapter.md`) is retained for its Agent-SDK research and auth finding.
